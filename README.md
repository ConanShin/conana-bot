...
-   **n8n-opencode**: 전체적인 워크플로우 오케스트레이션을 담당합니다. 텔레그램 메시지를 라우팅하고 각 전용 서비스에 API 요청을 보냅니다.
-   **opencode-proxy**: 로컬 OpenCode CLI를 실행하는 핵심 게이트웨이입니다. 대화 세션 관리, 일반적인 AI 처리, 이미지 분석 유틸리티 및 이메일 발송 기능을 담당합니다.
-   **naver-proxy**: 네이버 블로그 콘텐츠 생성 전용 서비스입니다. 고품질 글 작성을 위한 프롬프트 엔지니어링 및 텍스트 포맷팅 로직이 내장되어 있습니다.
-   **stock-proxy**: 주식 포트폴리오 분석 시스템입니다. 텔레그램 이미지 다운로드, 시각적 데이터 분석, 투자 전략 수립 및 리포트 생성을 하나의 파이프라인으로 통합 처리합니다.
-   **Cloudflare Tunnel**: 로컬 n8n 환경을 외부에 안전하게 노출시켜 텔레그램 웹훅을 수신 가능하게 합니다.
